...
public class TestMain {
	...
	public static final String APP_RUNNING = "ps -ax | grep apps";
	public static final String[] COMMANDS = {APP_RUNNING, CPU_MEM_SHELL, FILES_SHELL};
	...
	private static String disposeResultMessage(Map<String, String> result) {
		...
		for (String command : COMMANDS) {
			...
			} else if (command.equals(APP_RUNNING)){
				String[] strings = commandResult.split(LINE_SEPARATOR);
				buffer.append("启动项目包括").append(LINE_SEPARATOR);
				for (String line : strings) {
					if(line.contains("0.0.1")){
						line = line.substring(line.indexOf("/home/octopus/apps/octopus-")+27);
						line = line.substring(0, line.indexOf("-0.0.1"));
	...
	public static void main(String[] args) {
		Map<String, String> result = runDistanceShell(COMMANDS, "root", "123qwe$%^", "10.129.56.192");
